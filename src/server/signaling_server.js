...
const ROOM_RECONCILE_SWEEP_MS =
  1_000;

const ROOM_RECONCILE_RECHECK_MS =
  Math.max(
    1_000,
    Math.min(
...
const ROOM_RECONCILE_BATCH_SIZE =
  100;

const ROOM_RECONCILE_BACKFILL_SCAN_COUNT =
  500;
...
/*
 * connection generation -> 최초 disconnect 시점에 계산한 dueAtMs
 *
 * Redis 장애 때문에 같은 socket의 outer cleanup retry가 발생해도
 * grace deadline이 뒤로 밀리지 않도록 유지한다.
 *
 * 동일 peerId가 resume되어 새 socket generation을 만들더라도
 * old/new connection의 deadline은 서로 공유하지 않는다.
 */
 ...
 * 필요하지 않다.
 *
 * peerId가 아니라 connection generation을 추적해야
 * 동일 peerId의 newer resume socket과 충돌하지 않는다.
 */
...
let roomReconcileRunning =
  false;

let roomReconcileTimer =
  null;

let connectionCleanupShutdownDeadlineMs =
  null;
...
async function refreshOnePeer(
  ...
  try {
    ...
    console.error(
      `[presence] failed to inspect owner for peer ${peerId}:`,
      error,
    );
  }
  ...
  if (owner === null) {
    console.error(
      `[presence] peer ${peerId} lost its presence lease`,
    );
  } else if (
    typeof owner === 'string' &&
    ...
    console.error(
      `[presence] peer ${peerId} refresh failed after lease ownership changed`,
    );
  }

  /*
   * refresh()가 false를 반환했다는 것은 이 connection generation이
   * Redis presence lease를 더 이상 소유하지 않는다는 뜻이다.
   * periodic refresh 경로에서는 lease를 재등록하지 않는다.
   *
   * newer same-instance socket이 localPeers를 이미 takeover했다면
   * 위 generation check에서 빠져나오므로 새 socket을 닫지 않는다.
   */
...
async function sweepRoomReconcile() {
  if (
    shuttingDown ||
    roomReconcileRunning
  ) {
    return;
  }
    ...
    const scheduled =
      await roomMembership
        .reconcileOrphanedRooms({
          ...
          recheckMs:
            ROOM_RECONCILE_RECHECK_MS,

          limit:
            ROOM_RECONCILE_BATCH_SIZE,
        });

    if (Array.isArray(scheduled)) {
      for (
        let index = 0;
        index + 2 < scheduled.length;
        index += 3
      ) {
        console.log(
          `[room-reconcile] scheduled room ${scheduled[index]} for missing peer ${scheduled[index + 1]} at ${scheduled[index + 2]}`,
        );
      }
    }
  } catch (error) {
    console.error(
      '[room-reconcile] sweep failed:',
      error,
    );
  } finally {
    roomReconcileRunning =
...
function startRoomReconcileSweep() {
  void sweepRoomReconcile();
  ...
  roomReconcileTimer =
    setInterval(
      ...
      ROOM_RECONCILE_SWEEP_MS,
    );

  roomReconcileTimer.unref();
}
...
async function schedulePeerDisconnect(
  ...
  if (
    ...
    throw new TypeError(
      'connection is required to schedule peer disconnect',
    );
  }
   ...
   *
   * legacy 여부도 connection generation 단위로 판단한다.
   */
  if (
    legacyDisconnectConnections.has(
      connection,
    )
  ) {
...
function beginConnectionCleanupShutdown() {
  if (
    connectionCleanupShutdownDeadlineMs !==
    null
  ) {
    return connectionCleanupShutdownDeadlineMs;
  }
  ...
  if (
    ...
    throw new Error(
      'failed to calculate shutdown cleanup deadline',
    );
  }
...
function getConnectionCleanupShutdownRemainingMs() {
  if (
    connectionCleanupShutdownDeadlineMs ===
    null
  ) {
...
function shouldRetryConnectionCleanup() {
  const remainingMs =
    getConnectionCleanupShutdownRemainingMs();
...
async function cleanupConnection(
   ...
   */
  if (legacyConnection) {
    legacyDisconnectConnections.add(
      ws,
    );
  }
  ...
  try {
    while (true) {
      ...
      try {
        ...
        if (!shouldRetryConnectionCleanup()) {
          console.error(
            `[connection] durable cleanup retry deadline reached for peer ${remainingMeta.peerId}; deferring recovery to room reconciliation:`,
            error,
          );
    ...
    legacyDisconnectConnections.delete(
      ws,
    );
  ...
  const backfilledRooms =
    await roomMembership
      .backfillRoomWatch({
        ...
        scanCount:
          ROOM_RECONCILE_BACKFILL_SCAN_COUNT,
      });

  if (backfilledRooms > 0) {
    console.log(
      `[room-reconcile] backfilled ${backfilledRooms} room(s) into room-watch`,
    );
  }
...
async function closeWebSocketServer() {
   ...
   */
  const cleanupsFinished =
    await waitForConnectionCleanups();

  if (!cleanupsFinished) {
    console.warn(
      '[shutdown] connection cleanup deadline reached; orphan room recovery remains durable through room-watch reconciliation',
    );
  }
}
